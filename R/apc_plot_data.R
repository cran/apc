...
apc.data.sums	<- 	function(apc.data.list,data.type="r",average=FALSE,apc.index=NULL)
#	BN 17 Nov 2016
#	BN 15 Dec 2013
#	input:	apc.data.list		
...
#	sums.coh	<- colSums(trap,na.rm=TRUE)
	...
	#	take sums
	if(average==TRUE)	
	{	sums.age	<- rowMeans(trap,	na.rm=TRUE)
		sums.coh	<- colMeans(trap,	na.rm=TRUE)
		sums.per	<- colMeans(trap.ap,na.rm=TRUE)
	}
	if(average==FALSE)
	{	sums.age	<- rowSums(trap   ,na.rm=TRUE)	#ifelse(apply(is.na(trap   ),1,all),NA,rowSums(trap   ,na.rm=TRUE))
		sums.coh	<- colSums(trap   ,na.rm=TRUE)	#ifelse(apply(is.na(trap   ),1,all),NA,colSums(trap   ,na.rm=TRUE))
		sums.per	<- colSums(trap.ap,na.rm=TRUE)	#ifelse(apply(is.na(trap.ap),1,all),NA,colSums(trap.ap,na.rm=TRUE))
	}	
	#	return				
...
{	#	apc.plot.data.sums
	...
	{	sums.response	<- apc.data.sums(apc.data.list,data.type="r",average=FALSE,apc.index)
	...
	{
		sums.dose		<- apc.data.sums(apc.data.list,data.type="d",average=FALSE,apc.index)
	...
	{
		sums.dose		<- apc.data.sums(apc.data.list,data.type="m",average=FALSE,apc.index)
